...
const MindMapFlowContent = ({ data }) => {
  ...
  /**
   * Toggle channel expansion - only one channel can be expanded at a time
   */
  const toggleChannel = useCallback((channel) => {
    setExpandedChannels(prev => {
      ...
      if (!prev.has(channel)) {
        ...
        // Center on the expanded channel
        setTimeout(() => {
          if (reactFlowInstance) {
            const channelNode = reactFlowInstance.getNode(`channel-${channel}`);
            if (channelNode) {
        ...
        // When collapsing channel, fit the entire view
        setTimeout(() => {
          if (reactFlowInstance) {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    channels.forEach((channel, channelIndex) => {
      ...
      nodes.push({
        ...
        draggable: true,
        key: `${channel}-${isExpanded}-${channelPosition.x}-${channelPosition.y}`,
      });

      edges.push({
        id: `summary-to-${channel}`,
        source: 'summary',
        target: `channel-${channel}`,
        sourceHandle: 'summary-bottom',
        targetHandle: `${channel}-target-top`,
        label: `${data.channels[channel]?.length || 0} ideas`,
        style: { stroke: '#2c4bff', strokeWidth: 2 },
      });

      // Add idea nodes if channel is expanded
      if (isExpanded) {
        const channelIdeas = data.channels[channel] || [];
        const ideasToShow = channelIdeas.slice(0, 3);
        
        ideasToShow.forEach((idea, ideaIndex) => {
          const ideaId = `idea-${channel}-${ideaIndex}`;
          ...
          edges.push({
            id: `${channel}-to-${ideaId}`,
            source: `channel-${channel}`,
            target: ideaId,
            sourceHandle: `${channel}-source-bottom`,
