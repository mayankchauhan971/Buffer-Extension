...
const IdeaNode = ({ data, id }) => {
  const { idea, ideaIndex, channel, isFromTopChannel } = data;
  ...
  const handleCreatePost = (e) => {
    ...
    chrome.tabs.query({ 'active': true, 'currentWindow': true }, function(tabs) {
      ...
      // Format the idea content for posting
      const ideaContent = `${idea.idea || 'No content available'}

Why this might work: ${idea.rationale || 'No rationale provided'}

Pros: ${formatListItems(idea.pros)}
Cons: ${formatListItems(idea.cons)}`;
  ...
  const handleSaveIdea = (e) => {
    ...
    chrome.tabs.query({ 'active': true, 'currentWindow': true }, function(tabs) {
      ...
      // Format the idea content for saving
      const ideaContent = `${idea.idea || 'No content available'}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {channel.charAt(0).toUpperCase() + channel.slice(1)} Post Idea - {ideaIndex + 1}
      </div>
      ...
      <div style={{
        ...
        <p style={{ marginBottom: '14px' }}>
          ...
          <p>{idea.idea || 'No content available'}</p>
        </p>
        ...
        {/* Pros Paragraph */}
        <p style={{ marginBottom: '14px' }}>
          <strong style={{ color: '#28a745', fontSize: '14px' }}>✅ Pros:</strong> {formatListItems(idea.pros)}
        </p>

        {/* Cons Paragraph */}
        <p style={{ marginBottom: '16px' }}>
          <strong style={{ color: '#dc3545', fontSize: '14px' }}>❌ Cons:</strong> {formatListItems(idea.cons)}
        </p>
      </div>
